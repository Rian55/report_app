...
class _MainPage extends State<MainPage> {
  List<DropdownMenuItem<String>> get dropdownItems{
    List<DropdownMenuItem<String>> menuItems = [
      ...
      const DropdownMenuItem(value: "SABİT GÖREVLER", child: Text("Sabit Görevler")),
    ...
    Widget build(BuildContext context) {//<-----------------------------------IT'S HERE
      ...
      for(int i = 0; i < widget.allTasks.length; i++) {
        if (widget.allTasks[i].stage.contains(widget.stage) && !widget.stage.contains("Tümü") &&
        widget.allTasks[i].members.contains(widget.member)){
          widget.sortedTasks.add(widget.allTasks[i]);
        }else if(widget.stage.contains("Tümü") && widget.allTasks[i].members.contains(widget.member)){
          widget.sortedTasks.add(widget.allTasks[i]);
        }else if(widget.member == "" && widget.allTasks[i].stage.contains(widget.stage)){
          widget.sortedTasks.add(widget.allTasks[i]);
  ...
  Future letsTryAgain(BuildContext context) async{
    ...
    List<String> strs = [];
    for(var x in allData) strs.add(x.toString());
  ...
  /*Widget plsNoErr(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        var pendenciesList = snapshot.data!.docs.map((docs) => docs.data());
        //final re = RegExp(r'([^,|{|}\]\[])+');
        List<String> temp = [];
        for(var key in pendenciesList) temp.add(key.toString());
 ...
 Widget strToLV(List<String> arr){
      return ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: arr.length+2,
          itemBuilder: (BuildContext context, int index) {
            final re = RegExp(r'([,{}\[\]:])+');
            List<String> splitted = arr[index].split(re);
            String title = "";
            List<String> members = [];
            String dueDate = "";
            String lastActivity = "";
